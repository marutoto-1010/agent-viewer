...
- 📊 Mermaid図表対応（```mermaid ブロックをネイティブSVGレンダリング。初回描画時のみCDNから遅延ロード）
- 🔄 **横向き表示** — ツールバーの 🔄 で画面を90°回転。横長のMermaid図をスマホの長辺で読める（端末の回転ロックがONでも効く）
- 🌐 HTMLファイルのインラインプレビュー
...
- **Markdown**: 自前の軽量パーサー
- **Mermaid**: mermaid@11（jsDelivr CDN・Mermaidブロックを含むファイルを開いた時のみ遅延ロード）
  - `useMaxWidth:false` で**実寸描画**。画面幅に押し込むと文字が潰れて読めないため（例: 幅1998pxの図が390pxの画面で約1/5.6に縮小されていた）。幅が余る図だけ `margin:auto` で中央寄せし、はみ出す図は左端から表示して横スクロールさせる（`justify-content:center` だと左端がスクロールで到達不能になる）
- **ホスティング**: GitHub Pages
